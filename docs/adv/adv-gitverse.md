...
Из-за того, что в предыдущем шаге мы использовали флаг `-u`, наша локальная ветка `main` теперь считает своим основным (_дефолтным_) направлением GitVerse (`verse`):
...
1. Открыть репозиторий _learning-sql_ на GitVerse.
...
touch deploy.yaml
```
_Флаг `-p` позволяет создать всю цепочку вложенных папок за один раз и не выдает ошибку, если они уже существуют._
...
**Из плюсов**: использование GitVerse в качестве зеркала для локального репозитория понятно, работоспособно.

---
**Из минусов**: использовать GitVerse в качестве сайта идея так себе.
- Сайт _грузится ощутимо дольше_ чем на GitHub.
- Сборка сайта (деплой) _чаще заканчивается ошибкой_. Да, сервера GitHub тоже сбоят, но не так часто.
- Пожалуй **главное** – _поиск_ на собранном сайте в GitVerse _не работает_. Можете попробовать сами:
    
    GitVerse https://alexey-sm.gitverse.site/learning-sql/ \
    GitHub https://magus1968.github.io/learning-sql/
:::
