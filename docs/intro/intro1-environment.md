...
:class: simple
:icon: false
- Работая над этим проектом на двух разных машинах, я позволил себе в качестве эксперимента установить на основном компе с современным CPU `pip install polars`. Поэтому в таблице присутствуют две строки с версией `1.43.2`.
